...
try {
    // Fetch all main routes and join with buses
    $routes_list = $_conn_db->query("SELECT r.*, b.bus_name FROM routes r JOIN buses b ON r.bus_id = b.bus_id ORDER BY r.route_id DESC")->fetchAll(PDO::FETCH_ASSOC);
    
    if (!empty($routes_list)) {
        $route_ids = array_column($routes_list, 'route_id');
        $in_clause = implode(',', array_fill(0, count($route_ids), '?'));
        
        // Fetch all stops for all routes at once for efficiency
        ...
        // Fetch all schedules for all routes at once for efficiency
    ...
    $_SESSION['notif_desc'] = 'Could not fetch route data.';
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .schedule-timeline-content strong { display: block; font-size: 1.1em; }
        ...
        @media (max-width: 576px) { .page-header { flex-direction: column; align-items: flex-start; } .route-card-title h5 { font-size: 1.1rem; } }
    </style>
</head>
<body>
<div id="wrapper">
                        ...
                        <div class="route-card status-<?php echo $route['status']; ?>">
                            <div class="route-card-body">
                                <div class="route-card-title">
                                    <h5><?php echo htmlspecialchars($route['route_name']); ?></h5>
                                                ...
                                                <div class="schedule-day-block">
                                                    ...
                                                    <div class="schedule-timeline">
                                                        ...
                                                        try {
                                                            ...
                                                            if (isset($all_stops[$route['route_id']])) {
