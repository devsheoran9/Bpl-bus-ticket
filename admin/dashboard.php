                    ...
                    <div class="row g-4 mt-4">
                        <div class="col-lg-4">
                            <div class="data-list-card">
                                <div class="card-header">Live Bookings</div><?php foreach ($feeds['live_bookings'] as $b) {
                                                                                echo "<a href='booking_details.php?booking_id={$b['booking_id']}' class='data-list-item'><div class='item-info'><div class='title'>" . htmlspecialchars($b['route_name']) . "</div><div class='subtitle'>#" . htmlspecialchars($b['ticket_no']) . " by " . htmlspecialchars($b['booker']) . "</div></div><small class='text-muted'>" . date('h:i A', strtotime($b['created_at'])) . "</small></a>";
                                                                            }
                                                                            if (empty($feeds['live_bookings'])) echo "<div class='p-4 text-muted text-center'>No recent bookings.</div>"; ?>
                            </div>
                        </div>
                        <div class="col-lg-4">
                            <div class="data-list-card">
                                <div class="card-header">Busiest Routes (By Revenue)</div><?php foreach ($feeds['top_routes'] as $r) {
                                                                                                echo "<a href='view_bookings.php?route_id={$r['route_id']}' class='data-list-item'><div class='item-info'><div class='title'>" . htmlspecialchars($r['route_name']) . "</div><div class='subtitle'>{$r['booking_count']} bookings</div></div><span class='badge bg-success rounded-pill'>₹" . number_format($r['total_revenue']) . "</span></a>";
                                                                                            }
                                                                                            if (empty($feeds['top_routes'])) echo "<div class='p-4 text-muted text-center'>No data.</div>"; ?>
                            </div>
                        </div>
                        <div class="col-lg-4">
                            <div class="data-list-card">
                                <div class="card-header">Top Employees</div><?php foreach ($feeds['top_employees'] as $e) {
                                                                                echo "<a href='employee_bookings.php?employee_id={$e['id']}' class='data-list-item'><div class='item-info'><div class='title'>" . htmlspecialchars($e['name']) . "</div><div class='subtitle'>{$e['booking_count']} bookings</div></div><span class='badge bg-success rounded-pill'>₹" . number_format($e['total_sales']) . "</span></a>";
                                                                            }
                                                                            if (empty($feeds['top_employees'])) echo "<div class='p-4 text-muted text-center'>No sales data.</div>"; ?>
                            </div>
                        </div>
                    </div>
