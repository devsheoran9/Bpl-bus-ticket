...
function sendBookingEmail($booking_id, $recipient_email, $_conn_db) {
    ...
    try {
        // --- 1. Fetch ALL necessary data for the email template ---
        $stmt = $_conn_db->prepare("
            SELECT 
                b.ticket_no, b.travel_date, b.total_fare, b.origin, b.destination, b.contact_email,
                r.route_name,
                sch.departure_time,
        ...
        $replacements = [
            ...
            '{{account_info}}'      => '', // You can add content here if needed
        ];
        ...
        $mail->AltBody = 'Your booking is confirmed. Please view this email in an HTML-compatible client to see your ticket. Ticket No: ' . $booking['ticket_no'];

        $mail->send();
        return ['status' => 'success', 'message' => 'Email sent successfully.'];

    } catch (Exception $e) {
        error_log("Mailer Error: " . $mail->ErrorInfo); // Log the detailed mailer error
