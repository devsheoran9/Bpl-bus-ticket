<?php
// employee_bookings.php - Redesigned
...
try {
    ...
    if ($employee_id_filter) {
        $sql .= " AND b.booked_by_employee_id = ?";
    ...
    foreach ($bookings as $booking) {
        if ($booking['booker_type'] === 'Employee') {
            ...
            // User bookings are always ONLINE and considered "collected"
...
<body>
    <div id="wrapper">
        ...
        <div class="main-content">
            ...
            <div class="container-fluid">
                ...
                <div class="card filter-card mb-4">
                    <div class="card-body">
                        <form method="GET" class="row g-3 align-items-end">
                            <div class="col-12 col-sm-6 col-lg-3"><label class="form-label fw-bold">Employee</label><select name="employee_id" class="form-select">
                                    <option value="">All Employees</option><?php foreach ($employees as $emp): ?><option value="<?php echo $emp['id']; ?>" <?php echo ($employee_id_filter == $emp['id']) ? 'selected' : ''; ?>><?php echo htmlspecialchars($emp['name']); ?></option><?php endforeach; ?>
                                </select></div>
                            <div class="col-12 col-sm-6 col-lg-3"><label class="form-label fw-bold">Bus</label><select name="bus_id" class="form-select">
                                    <option value="">All Buses</option><?php foreach ($buses as $bus): ?><option value="<?php echo $bus['bus_id']; ?>" <?php echo ($bus_id_filter == $bus['bus_id']) ? 'selected' : ''; ?>><?php echo htmlspecialchars($bus['bus_name']); ?></option><?php endforeach; ?>
                                </select></div>
                            <div class="col-12 col-sm-6 col-lg-2"><label class="form-label fw-bold">Route</label><select name="route_id" class="form-select">
                                    <option value="">All Routes</option><?php foreach ($routes as $route): ?><option value="<?php echo $route['route_id']; ?>" <?php echo ($route_id_filter == $route['route_id']) ? 'selected' : ''; ?>><?php echo htmlspecialchars($route['route_name']); ?></option><?php endforeach; ?>
                                </select></div>
                            <div class="col-12 col-sm-6 col-lg-2"><label class="form-label fw-bold">Date Range</label><input type="text" name="date_range" class="form-control date-range-picker" placeholder="Select Dates"></div>
                            <input type="hidden" name="date_from" id="date_from" value="<?php echo $date_from_filter; ?>">
                            <input type="hidden" name="date_to" id="date_to" value="<?php echo $date_to_filter; ?>">
                            <div class="col-12 col-lg-2 d-flex gap-2"><button type="submit" class="btn btn-primary w-100"><i class="fas fa-filter me-1"></i> Filter</button><a href="employee_bookings.php" class="btn btn-light border w-100">Reset</a></div>
                        </form>
                    </div>
                </div>
                ...
                <!-- Online Customers Report Card -->
                <?php if ($user_report['total_bookings'] > 0): ?>
                    <div class="report-card">
                        <div class="report-header">
                            <div class="avatar avatar-user"><i class="fas fa-globe"></i></div>
                            <div class="report-info">
                                <h4>Online Customer Bookings</h4>
                                <p>Bookings made by customers via website/app.</p>
                            </div>
                        </div>
                        <div class="summary-grid">
                            <div class="summary-box">
                                <span class="label">Total Bookings</span>
                                <div class="value text-total"><?php echo $user_report['total_bookings']; ?></div>
                            </div>
                            <div class="summary-box">
                                <span class="label">Total Online Sales</span>
                                <div class="value text-cash-collected">₹<?php echo number_format($user_report['total_online'], 2); ?></div>
                            </div>
                        </div>
                ...
                <?php foreach ($employee_report as $employee_id => $data): ?>
                    <div class="report-card">
                        <div class="report-header">
                            ...
                            <div class="report-info">
                                <h4><?php echo htmlspecialchars($data['employee_name']); ?></h4>
                                <p>Sales report based on selected filters.</p>
                            </div>
                            <?php if ($data['cash_due'] > 0): ?>
                            <div class="cash-due-box">
                                <span class="label">Cash to be Collected</span>
                                <div class="amount">₹<?php echo number_format($data['cash_due'], 2); ?></div>
                            </div>
                            <?php endif; ?>
                        </div>
                        <div class="summary-grid">
                    ...
                    <div class="modal fade" id="detailsModal<?php echo $employee_id; ?>" tabindex="-1">
                        <div class="modal-dialog modal-xl modal-dialog-scrollable">
                            <div class="modal-content">
                                ...
                                <div class="modal-body p-0"><table class="table table-striped table-hover details-table mb-0">
                                    <thead class="sticky-top"><tr><th>Ticket #</th><th>Travel Date</th><th>Route</th><th>Bus</th><th>Payment</th><th>Status</th><th>Fare</th><th>Action</th></tr></thead>
                                    <tbody><?php foreach ($data['bookings'] as $booking): ?><tr>
                                        ...
                                        <td class="fw-bold">₹<?php echo number_format($booking['total_fare'], 2); ?></td>
                                        <td><a href="generate_ticket?booking_id=<?php echo $booking['booking_id']; ?>" class="btn btn-sm btn-outline-primary" target="_blank"><i class="fas fa-eye"></i> View</a></td>
                ...
                <div class="modal fade" id="detailsModalUser" tabindex="-1">
                    <div class="modal-dialog modal-xl modal-dialog-scrollable">
                        <div class="modal-content">
                            ...
                            <div class="modal-body p-0"><table class="table table-striped table-hover details-table mb-0">
                                <thead class="sticky-top"><tr><th>Ticket #</th><th>Booked By</th><th>Travel Date</th><th>Route</th><th>Bus</th><th>Status</th><th>Fare</th><th>Action</th></tr></thead>
                                <tbody><?php foreach ($user_report['bookings'] as $booking): ?><tr>
                                    <td><b><?php echo htmlspecialchars($booking['ticket_no']); ?></b></td>
                                    <td><?php echo htmlspecialchars($booking['booker_name']); ?></td>
                                    <td><?php echo date('d-M-Y', strtotime($booking['travel_date'])); ?></td>
                                    <td><?php echo htmlspecialchars($booking['route_name']); ?></td>
                                    <td><?php echo htmlspecialchars($booking['bus_name']); ?></td>
                                    <td><span class="text-success fw-bold"><i class="fas fa-check-circle me-1"></i>Online Payment</span></td>
                                    <td class="fw-bold">₹<?php echo number_format($booking['total_fare'], 2); ?></td>
                                    <td><a href="generate_ticket?booking_id=<?php echo $booking['booking_id']; ?>" class="btn btn-sm btn-outline-primary" target="_blank"><i class="fas fa-eye"></i> View</a></td>
                                </tr><?php endforeach; ?></tbody>
                            </table></div>
    ...
    $(document).ready(function() {
        flatpickr(".date-range-picker", {
            mode: "range",
            dateFormat: "Y-m-d",
            defaultDate: ["<?php echo $date_from_filter; ?>", "<?php echo $date_to_filter; ?>"],
            onChange: function(selectedDates, dateStr, instance) {
                if (selectedDates.length === 2) {
                    $('#date_from').val(instance.formatDate(selectedDates[0], "Y-m-d"));
                    $('#date_to').val(instance.formatDate(selectedDates[1], "Y-m-d"));
                } else {
                    $('#date_from').val('');
                    $('#date_to').val('');
                }
        ...
        $('.collect-all-cash-btn').on('click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: 'function/backend/report_actions.php', // Ensure this is the correct endpoint
                        type: 'POST',
                        data: filterData,
                        dataType: 'json',
